...
describe("submitGuess service", () => {
    ...
    it("returns 400 if guess is not boy or girl", async () => {
        ...
        expect(result).toEqual({ success: false, status: 400, message: "guess must be boy or girl" });
    });
    ...
    it("returns 404 if scan does not exist", async () => {
        ...
        expect(result).toEqual({ success: false, status: 404, message: "scan not found" });
    });
    ...
    it("returns 409 if scan is already revealed", async () => {
        ...
        const result = await submitGuess({ scanId: "123", userId: "user1", guess: Genders.Girl });
        expect(result).toEqual({ success: false, status: 409, message: "this scan has already been revealed" });
    });
    ...
    it("returns 409 if user tries to guess on their own scan", async () => {
        ...
        const result = await submitGuess({ scanId: "123", userId: "user1", guess: Genders.Girl });
        expect(result).toEqual({ success: false, status: 409, message: "you cannot guess on your own scan" });
    });
    ...
    it("returns 409 if user already guessed on this scan", async () => {
        ...
        const result = await submitGuess({ scanId: "123", userId: "user1", guess: Genders.Girl });
        expect(result).toEqual({ success: false, status: 409, message: "you already guessed on this scan" });
    });
